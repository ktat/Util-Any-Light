...
use UtilAnyList {'-list' => ['min'], -string => ['camelize']}, {prefix => 1};
ok(defined &list_min, "");
...
use UtilAnyList -list => ['uniq', 'min'], {prefix => 1};
...
use UtilAnyList -list => {-prefix => "l_"};
...
use UtilAnyList -list => {uniq => {-as => 'listuniq'}};
is_deeply([listuniq qw/1 0 1 2 3 3/], [1,0,2,3]);

package DD;
use Test::More;

use UtilAnyList -list => {uniq => {-as => 'li_uniq'}, -prefix => "l_"};
...
use UtilAnyList -list => {uniq => {-as => 'li_uniq'}, -prefix => "ll_"}, {smart_rename => 1};
is_deeply([li_uniq qw/1 0 1 2 3 3/], [1,0,2,3]);
